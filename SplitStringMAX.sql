CREATE FUNCTION [Tools].[SplitStringMAX] (@String    VARCHAR(MAX)
...
AS
  ...
  WITH [CTE_10]
       ...
       -------------------
       /* 100,000,000 rows max (all 1s) - this limits the number of elements to 10k (which is fine, because our datatype only accepts 8k characters) */
       [CTE_100000000]
       AS (SELECT [Number] = 1
           FROM [CTE_10000] [a]
